...
public class DBF {
    private static void makeValue(String key, String value, String database) {
        try {
            ...
            file.mkdir();
            FileWriter fileWriter = new FileWriter("DBF/" + database, true);
    ...
    private static void closeObject(String database) {
        try {
            File file = new File("Databases en Files");
            file.mkdir();
            FileWriter fileWriter = new FileWriter("DBF" + database, true);
    ...
    public static void makeObject(String key1, String value1,String key2, String value2,String key3, String value3,String key4, String value4,String key5, String value5, String key6, String value6, String database) {
        makeValue(key1, value1, database);
        makeValue(key2, value2, database);
        makeValue(key3, value3, database);
        makeValue(key4, value4, database);
    ...
    public static void addKey(String key, String database) {
        try {
            FileWriter fileWriter = new FileWriter("DBF/temp");
            BufferedReader bufferdReader = new BufferedReader(new FileReader("DBF/" + database));
    ...
    private static void copyTempTo(String database) {
        try {
            FileWriter fileWriter = new FileWriter("DBF/" + database);
            BufferedReader bufferdReader = new BufferedReader(new FileReader("DBF/temp"));
            String line = bufferdReader.readLine();
            while (line != null) {
                fileWriter.append(line);
                fileWriter.append("\n");
    ...
    public static ArrayList<String> getArrayValue(String privateKey, String keyValue, String key, String database){
        ...
        x = DBF.stripValue(privateKey, database);
        for (int i = 0; i < x.size(); i++) {
            if (x.get(i).equals(keyValue)) {
                x = DBF.stripValue(key, database);
                First = x.get(i).split("-");
